...
"""
    ...
    # ── 반의도 중간 → 걱정 파악 후 상황판 유도 ──
    if As >= 0.45:
        return {
            'action': 'ANTI_INTENT',
            'reason': f"반의도 감지 As={As}",
            'rule': """
사용자가 고민하고 있습니다.
다음 규칙을 반드시 지키세요:

규칙 1: 공감 1줄 + 질문 1개만.
예: "내구성이 걱정되시는군요! 어떤 상황에서 주로 사용하실 예정인가요?"

규칙 2: 사용자가 걱정/상황을 말하면:
- 공감 1줄
- 해소 또는 팁 1줄
- 반드시 "그럼 조건 골라볼까요? 😊" 로 끝내기
- 마지막 줄에 BOARD_READY 출력
...
규칙 3: 절대 금지
- 추가 질문 금지 (사용자가 한번이라도 답했으면 바로 BOARD_READY)
- 3줄 이상 금지
- 제품 추천 금지
"""
        }
